...
- Роутинг теперь сначала предпочитает недавние окна, а поиск по всем top-level окнам использует только как fallback для global-shortcut.
...
- Роутинг выполняется в два этапа:
  1. Сначала идет поиск по recent/history окнам: кандидаты проверяются по недавности (`targetSearchDepth`), а внутри каждого окна rules проверяются по убыванию `prio`. На этом этапе допускаются и global, и non-global actions.
  2. Если recent/history ничего не дал, приложение сканирует все top-level окна, но только для routes, у которых shortcut использует `send: "global"`. Приоритет rules важнее порядка перечисления окон.
- Если окно совпало с target rule, но для текущего trigger нет route, поиск продолжается по следующим rules/окнам.
- После первого успешно выполненного action обработка завершается.
